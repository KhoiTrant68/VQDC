...
from modules.dynamic.dynamic_utils import draw_triple_grain_256res_color


class TripleGrainVQModel(nn.Module):
    """
    ...
    def log_images(self, batch: dict, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        log["grain_map"] = draw_triple_grain_256res_color(
            images=x.clone(), indices=grain_indices
        )
        x_entropy = x_entropy.sub(x_entropy.min()).div(
            max(x_entropy.max() - x_entropy.min(), 1e-5)
        )
        log["entropy_map"] = draw_triple_grain_256res_color(
            images=x.clone(), indices=x_entropy, scaler=0.7
        )
        return log

    def get_code_emb_with_depth(self, code: torch.Tensor) -> torch.Tensor:
        """
