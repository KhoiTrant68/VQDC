...
class VQEmbedding(nn.Embedding):
    ...
    def compute_distances(self, inputs):
        return torch.cdist(inputs.reshape(-1, self.weight.shape[-1]), self.weight[:-1, :]).reshape(*inputs.shape[:-1], -1)
    ...
    def _update_buffers(self, vectors, idxs):
        ...
        cluster_size = torch.zeros(n_embed, dtype=torch.long, device=vectors.device).scatter_add_(0, idxs, torch.ones_like(idxs, dtype=torch.long))
        vectors_sum_per_cluster = torch.zeros(n_embed, embed_dim, device=vectors.device).scatter_add_(0, idxs.unsqueeze(1).expand(-1, embed_dim), vectors)
        ...
        if self.restart_unused_codes:
            ...
            if num_unused > 0:
                ...
                random_indices = torch.randperm(vectors.shape[0], device=vectors.device)[:num_unused]
    ...
    def _update_embedding(self):
        ...
        self.weight[:-1, :].copy_(self.embed_ema / normalized_cluster_size.reshape(-1, 1))
...
class VectorQuantize2(nn.Module):
    def __init__(self,
        ...
        nn.init.kaiming_uniform_(self.codebook.weight, a=0, mode='fan_in', nonlinearity='linear')
    ...
    def forward(self, x, codebook_mask=None, *args, **ignorekwargs):
        ...
        if codebook_mask is not None:
            ...
            loss = self.beta * F.mse_loss(x_q.detach(), x, reduction='none') * codebook_mask.unsqueeze(-1) + F.mse_loss(x_q, x.detach(), reduction='none') * codebook_mask.unsqueeze(-1)
            loss = loss.mean() 
        else:
            loss = self.beta * F.mse_loss(x_q.detach(), x) + F.mse_loss(x_q, x.detach())
    ...
    def get_soft_codes(self, x, temp=1.0, stochastic=False):
        ...
        if stochastic:
            code = torch.multinomial(soft_code.flatten(0, -2), 1).reshape(*soft_code.shape[:-1])
        else:
