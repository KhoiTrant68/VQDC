...
class PositionAwareSOSProvider(AbstractEncoder):
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, ...]:
        ...
        c_medium = torch.full((batch_size, 1), self.medium_sos, device=device, dtype=torch.long) if self.medium_sos is not None else None
        ...
        c_pos_medium = torch.full((batch_size, 1), self.medium_pos_sos, device=device, dtype=torch.long) if self.medium_pos_sos is not None else None
        ...
        if self.activate_seg:
            ...
            c_seg_medium = torch.full((batch_size, 1), self.medium_seg_sos, device=device, dtype=torch.long)
            c_seg_fine = torch.full((batch_size, 1), self.fine_seg_sos, device=device, dtype=torch.long)
            return c_coarse, c_medium, c_fine, c_pos_coarse, c_pos_medium, c_pos_fine, c_seg_coarse, c_seg_medium, c_seg_fine

        return c_coarse, c_medium, c_fine, c_pos_coarse, c_pos_medium, c_pos_fine, None, None, None
...
def main():
    ...
    pos_aware_provider = PositionAwareSOSProvider(
        coarse_sos=1, coarse_pos_sos=2, medium_sos=3, medium_pos_sos=4,
        fine_sos=5, fine_pos_sos=6, coarse_seg_sos=7, medium_seg_sos=8, fine_seg_sos=9
    )
    input_tensor = torch.randn(2, 10)  # Batch size of 2, arbitrary input size
    output = pos_aware_provider.encode(input_tensor)
    print("Input shape:", input_tensor.shape)
    print("Output:", output)
    print("Explanation: Should return 9 tensors with shape (2, 1), containing the specified SOS tokens.")
    ...
    class_content_provider = ClassForContentOnlyPositionAwareSOSProvider(
        n_classes=10, threshold=100, coarse_pos_sos=2, medium_pos_sos=4, fine_pos_sos=6,
        coarse_seg_sos=7, medium_seg_sos=8, fine_seg_sos=9
    )
    class_input = torch.tensor([3, 7])  # Two class labels
    output = class_content_provider.encode(class_input)
    print("Input:", class_input)
    print("Output:", output)
    print("Explanation: Should return 9 tensors. The first three contain class labels + threshold (103 and 107),")
    ...
    class_aware_provider = ClassAwareSOSProvider(
        ...
        threshold_medium_position=300, threshold_fine_position=400,
        coarse_seg_sos=5, medium_seg_sos=6, fine_seg_sos=7
    )
    class_input = torch.tensor([3, 7])  # Two class labels
    output = class_aware_provider.encode(class_input)
    print("Input:", class_input)
    print("Output:", output)
    print("Explanation: Should return 9 tensors. The first six contain class labels + respective thresholds,")
